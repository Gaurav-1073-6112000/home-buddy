...
router.get("/cart", isLoggedIn, async function (req, res, next) {
  ...
  cartData.forEach((data) => {
    totalCartPrice += parseInt(data.price);
  });
  // console.log(totalCartPrice);
  res.render("cart", { data: cartData, totalCartPrice: totalCartPrice });
});
...
router.get("/remove/:orderId", isLoggedIn, async function (req, res, next) {
  await orderModel.findOneAndDelete({ _id: req.params.orderId });
  res.redirect("/cart");
});
...
var instance = new Razorpay({
  // key_id: "rzp_test_CC0bH6lM5ifK8S",
  // key_secret: "mctDAybjM4qmaURXAzqumMNs",
...
router.post("/create/:id", async function (req, res, next) {
  let cartData = await orderModel.find({ userId: req.params.id });
  let totalCartPrice = 0;
  cartData.forEach((data) => {
    totalCartPrice += parseInt(data.price);
  });
  var options = {
    amount: totalCartPrice * 100, // amount in the smallest currency unit
    currency: "INR",
    receipt: "order_rcptid_11",
  };
  instance.orders.create(options, function (err, order) {
...
router.post("/api/payment/verify", (req, res) => {
  ...
  var crypto = require("crypto");
  var expectedSignature = crypto
    // .createHmac("sha256", "mctDAybjM4qmaURXAzqumMNs")
